# Session 3-1 - 추측 대신 측정 가능한 완료 기준 세우기

> "잘 된 것 같다"는 AI Agent 말은 위험 신호입니다. 
> 작업을 시작하기 전에 무엇을 보면 완료라고 말할지 먼저 정합니다.

1. 코드를 "아름답게" 만들어줘.
> Clean Code(클린코드) 기준에 맞춰서 만들어줘.
==> 함수는 1개의 기능만을 포함하고, 함수 내에 10줄 이상을 작성하지 않습니다.

숫자로 표현 가능
-> 코드를 돌려도 똑같은 결과를 얻을 수 있다.

- 작업 요청을 목표, 제약, `완료 기준` 세 칸으로 나누어 적습니다.
- 완료 기준에는 사람이 직접 눈으로 확인할 증거를 적어도 하나 둡니다.(의미적인 증거)
- 테스트가 가능한 작업이라면 실패 테스트부터 먼저 만들게 합니다. (TDD)
> 어떤 테스트를 만들어야하는가?

* REST API Endpoint 1개 구성
- GET /items
- JSONL 타입으로 데이터를 리턴해야함.
- 만약, 내부 데이터가 10개 이상이면 페이지네이션을 고려해야함
 (또는 데이터 개수가 너무 많다면, Cursor 방식으로 변경을 고려해야함)
- 한 유저의 요청이 너무 짧은 시간에 많이 몰리지 않도록 해야함.
 (요청이 너무 자주 많이 온다면, Cahcing 고려해야할 수도 있음)
- 엣지 케이스를 충분히 고려할 것.
...
3. 완료 기준에 "테스트가 통과한다", "문서가 갱신된다", "사람이 확인한다" 중 어떤 증거가 들어갈 수 있을지 고민
...
Codex 공식 가이드는 작업 요청에 목표, 컨텍스트, 제약, 완료 기준을 함께 넣으라고 권합니다. Codex App에서는 작업 카드 본문 자체에 이 4가지를 명시해 두면, 다른 사람이 같은 카드를 가져가도 같은 기준으로 작업이 진행됩니다.

테스트가 가능한 변경이라면 "먼저 실패 테스트를 만들고, 통과시킬 최소 수정만 하라"는 한 줄을 끝에 두는 것이 효과가 큽니다.
...
`Done when` 카드를 검증 명령으로 묶어 둡니다. 
같은 카드를 받으면 누구나 같은 명령으로 통과 여부를 확인할 수 있어야 합니다.
