...
For a Korean project introduction, read [`PROJECT_OVERVIEW.md`](PROJECT_OVERVIEW.md). To explain Gajae-Code as an AI Agent Harness, read [`AI_AGENT_HARNESS_VIEW.md`](AI_AGENT_HARNESS_VIEW.md). For a source-level idea map, read [`ARCHITECTURE.md`](ARCHITECTURE.md). For a file-by-file Korean source walkthrough, read [`SOURCE_WALKTHROUGH.md`](SOURCE_WALKTHROUGH.md). For Mermaid diagrams of the GitNexus source graph, read [`SOURCE_GRAPH_DIAGRAMS.md`](SOURCE_GRAPH_DIAGRAMS.md). These documents explain the workflow-first harness design, the main `packages/coding-agent/` runtime, and the support-package boundaries.
