...
## AI ENTRY PROTOCOL: GITNEXUS FIRST

For Codex, Claude Code, and any other AI coding agent entering this project, GitNexus is the first code-intelligence surface.

- Before answering architecture, ownership, dependency, call-flow, or "how does this work?" questions, read the GitNexus project context and prefer `query({query: "..."})` over raw grep for unfamiliar code paths.
- Before editing any function, class, method, hook, tool, installer, CLI command, or exported symbol, run GitNexus impact analysis for the target symbol and state the blast radius: direct callers, affected processes, and risk level.
- Before refactors or renames, use GitNexus context/impact first; do not perform broad find-and-replace renames.
- Before completion or commit, run GitNexus change detection and confirm that only the expected symbols, files, and execution flows changed.
- Use the generated source-only wiki as the first human-readable map when the question is broad: `.gitnexus/wiki/index.html` and `.gitnexus/wiki/overview.md`.
- If GitNexus is missing or stale, refresh it from this project root with `node .gitnexus/run.cjs analyze` before doing deeper manual exploration.
...
This project is indexed by GitNexus as **oh-my-openagent-part7** (35134 symbols, 72682 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/oh-my-openagent-part7/context` | Codebase overview, check index freshness |
| `gitnexus://repo/oh-my-openagent-part7/clusters` | All functional areas |
| `gitnexus://repo/oh-my-openagent-part7/processes` | All execution flows |
| `gitnexus://repo/oh-my-openagent-part7/process/{name}` | Step-by-step execution trace |
